<template>
    <h1 class="title ms-4 text-center">Cập nhật sản phẩm</h1> 
            ...
            <div class="col-lg-9 p-5 pt-0 pe-0">
              ...
              <div class="row">
                ...
                <select class="form-select" v-model="form.type" aria-label="Default select example">
                    ...
                    <option value="Phone" disabled>iPhone</option>
                    <option value="iPad" disabled>iPad</option>
                    <option value="iPod" disabled>iPod</option>
                    <option value="Laptop" disabled>Laptop</option>
                </select>
...
const updateProduct = async () => {
  ...
  try {
    ...
    if (kiemtra) {
      ...
      await axios.delete(`${API_URL}${dbjson}/${form.value.id}`);

      alert("Sản phẩm đã tồn tại, số lượng được cập nhật!");
    } else {
      await axios.put(`${API_URL}${dbjson}/${form.value.id}`, form.value);
