...
const API_URL = "http://localhost:3000/ipads";

const fetchProducts = async () => {
  try {
    const response = await fetch(API_URL);
    ipads.value = await response.json();
  } catch (error) {
...
<template >
    ...
    <!-- Banner -->
    <video src="/images/banner-ipad.mp4.mp4" type="video/mp4" width="100%" height="460px" autoplay="autoplay" muted="" loop="">
    </video>
     ...
     <div class="container mx-auto py-5">
      ...
      <img :src="ipad.image" :alt="ipad.name" class="product-image" />
      <h2 class="product-name">{{ ipad.name }}</h2>
      
     
      <p class="product-price">{{ formatCurrency(ipad.price) }}</p>
      
      <button  @click="upchitiet(ipad.id, ipad.type)" class="product-button">
        Mua hàng
      </button>
    </div>
  </div>
