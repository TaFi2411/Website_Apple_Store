...
const API_URL = "http://localhost:3000/ipods";

const fetchProducts = async () => {
  try {
    const response = await fetch(API_URL);
    ipods.value = await response.json();
  } catch (error) {
...
<template >
    ...
    <!-- Banner -->
    <video src="/images/banner-ipod.mp4.mp4" type="video/mp4" width="100%" height="460px" autoplay="autoplay" muted="" loop="">
    </video>
     ...
     <div class="container mx-auto py-5">
      ...
      <img :src="ipod.image" :alt="ipod.name" class="product-image" />
      <h2 class="product-name">{{ ipod.name }}</h2>
      
     
      <p class="product-price">{{ formatCurrency(ipod.price) }}</p>
      
      <button  @click="upchitiet(ipod.id, ipod.type)" class="product-button">
        Mua hàng
      </button>
    </div>
  </div>
